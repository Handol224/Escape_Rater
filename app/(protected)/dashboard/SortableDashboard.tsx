...
const CATEGORY_KEYS: CategoryKey[] = ['puzzles', 'story_theme', 'atmosphere', 'difficulty', 'game_master']
...
export default function SortableDashboard({ rankings, sortLabels, stats }: Props) {
  ...
  // Best in each category (for badges)
  ...
  if (ratedRankings.length > 0) {
    const bestOverall = ratedRankings.reduce((a, b) => b.overall > a.overall ? b : a)
    bestIn.overall = bestOverall.room.id
    for (const key of CATEGORY_KEYS) {
      const best = ratedRankings.reduce((a, b) => b[key] > a[key] ? b : a)
      bestIn[key] = best.room.id
    }
  }
    ...
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          {
            label: 'Escape rate',
            value: stats.escapeRate !== null ? `${stats.escapeRate}%` : '—',
          },
        ].map(({ label, value }) => (
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-right shrink-0">
                    <div className="text-3xl font-bold">
                      <ScoreBadge value={r.overall} highlight={isTopOverall} />
                    </div>
                ...
                <div className="mt-4 grid grid-cols-5 gap-2">
                  {CATEGORY_KEYS.map(key => (
                    ...
                    >
                      ...
                      <ScoreBadge value={r[key] ?? 0} highlight={bestIn[key] === r.room.id && r.total_ratings > 0} />
                    </div>
                  ))}
                </div>
              </div>
